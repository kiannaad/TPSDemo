...
namespace Animancer
{
        ...
        IAnimationClipCollection
    {
        ...
        public void InitializeGraph()
        {
            ...
            // Graph 采用懒初始化：第一次访问 Graph/Layers/States 或调用 Play 时才创建，
            // 因而组件存在并不代表底层 PlayableGraph 已经存在。
            TryGetAnimator();

            AnimancerGraph.SetNextGraphName(name + " (Animancer)");
            _Graph = new(_Transitions?.Library);
            // Animancer 负责组织 Playable，Animator 仍是最终姿势的绑定与输出目标。
        ...
        public AnimancerState Play(AnimationClip clip)
            // 正常播放主链：先按 Clip 查找或创建 State，再交给默认 Layer 处理同层互斥和权重。
            // 注意：这里要求 Clip 已经加载；按需加载应当在进入此方法之前完成。
