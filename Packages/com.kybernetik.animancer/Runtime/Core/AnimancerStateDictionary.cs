...
namespace Animancer
{
        ...
        IEnumerable<AnimancerState>
    {
        ...
        {
            // Key 标识可复用的“播放状态”，Clip 是该状态实际采样的资源。
            // 默认两者是同一个 AnimationClip；按需加载改造可以改用资源地址作为稳定 Key。
            if (TryGet(key, out var state))
            {
                // 命中时复用旧 State/Playable，因此会保留旧时间等运行状态，而不是重新创建播放实例。
            ...
            {
                // 首次播放该 Key 才创建 ClipState；创建过程会把 State 注册到 Graph 并创建 Playable。
