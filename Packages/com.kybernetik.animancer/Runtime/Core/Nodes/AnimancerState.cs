...
namespace Animancer
{
        ...
        ICopyable<AnimancerState>
    {
        ...
        private void AddToNewGraph()
        {
            if (Graph != null)
            {
                // 先注册托管 State，再创建其原生 Playable，使 Graph 成为两者共同的生命周期所有者。
        ...
        protected internal void SetIsPlaying(bool isPlaying)
        {
            ...
            {
                // IsPlaying 只控制时间是否推进；一个暂停的 State 仍可能因 Weight > 0 影响最终姿势。
                if (_IsPlaying)
                    _Playable.Play();
                else
                    _Playable.Pause();
        ...
        {
            // Stop 只暂停并归零时间，不会释放 Clip 引用。真正卸载动画资源必须走 Destroy，
            // 先从 Layer/StateDictionary 移除并销毁 Playable，再由外部资源层释放 AssetHandle。
            if (Parent != null)
            {
                Parent.OnRemoveChild(this);
                Parent = null;
