...
namespace Animancer
{
        ...
        ICopyable<AnimancerLayer>
    {
            ...
            AnimancerState state)
        {
            ...
            // 立即播放会取消当前过渡，避免旧 FadeGroup 在后续帧继续改写目标状态的权重。
            CurrentState?.FadeGroup?.Cancel();

            // 确保 State 属于当前 Layer；必要时会挂接 State，并把其 Playable 接到 Layer Mixer。
            state = GetOrCreateState(state);

            CurrentState = state;

            // 普通 Play 是同层互斥播放：停止除目标之外的所有活跃 State。
            // CrossFade 走下方另一个重载，不会在这一刻直接清空旧状态。
            for (int i = ActiveStatesInternal.Count - 1; i >= 0; i--)
            {
            ...
            // 播放状态、活跃集合和 Mixer 输入权重是三个独立概念，这里需要同时更新。
